...
// Web Analytics was switched on for tany4.com on this day; earlier days have no data
// rather than no visitors
export const TRACKING_SINCE = "2026-09-15";
...
// Query filter that leaves the link-click and download views out
const PAGES_ONLY = "not startswith(requestPath,'/out/') and not startswith(requestPath,'/download/')";
const downloads = (paths) =>
  paths.filter((p) => p.requestPath.startsWith("/download/")).reduce((sum, p) => sum + p.pageviews, 0);
...
// week reads as quiet days rather than no days. Days before tracking started are left out.
function everyDay(r, rows) {
  ...
  for (let t = Date.parse(r.since); t <= Date.parse(r.until); t += 24 * 60 * 60 * 1000) {
    ...
    if (date.slice(0, 10) < TRACKING_SINCE) continue;
    days.push({ date, pageviews: pageviews.get(date.slice(0, 10)) || 0 });
  }
  return days;
}

// Post titles by URL, so the email can name posts rather than show long paths
function postTitles() {
  const dir = path.join(process.cwd(), "posts");
  const titles = new Map();
  for (const file of fs.readdirSync(dir)) {
    if (!file.endsWith(".md")) continue;
    const { title } = matter(fs.readFileSync(path.join(dir, file), "utf8")).data;
    if (title) titles.set(`/blog/posts/${file.replace(/\.md$/, "")}`, title);
  }
  return titles;
}
...
export async function getVercelWeek(r) {
  ...
  const [current, before, days, baselineDays, referrers, countries] = await Promise.all([
    totals(r),
    totals(previous),
    query("aggregate", { ...window(r), by: "day", filter: PAGES_ONLY }),
    query("aggregate", { ...window(baseline), by: "day", filter: PAGES_ONLY }),
  ...
  return {
    ...
    downloads: downloads(current.paths),
    previousDownloads: downloads(before.paths),
    trackedWholeWeek: r.since.slice(0, 10) >= TRACKING_SINCE,
    days: everyDay(r, days),
    baselineDays: everyDay(baseline, baselineDays),
    pages: current.paths
      .filter((p) => !isVirtualPath(p.requestPath))
      .slice(0, 10)
      .map((p) => ({ name: p.requestPath, title: titles.get(p.requestPath), pageviews: p.pageviews })),
