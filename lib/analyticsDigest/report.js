  ...
  if (cloudflare) {
    ...
    sections.push({
      ...
      lines: [
        ...
        cloudflare.current.loadTimeMs === null
          ? "Median page load time: no data"
          : `Median page load time: ${number(cloudflare.current.loadTimeMs)} ms (${vsLastWeek(cloudflare.current.loadTimeMs, cloudflare.previous.loadTimeMs)})`,
