...
// Gmail turns anything that looks like a domain into a link; a zero-width space
// after each dot stops that without changing what's shown
const noAutolink = (html) => html.replace(/\./g, ".&#8203;");
...
export function buildReport({ label, shortLabel = label, vercel, cloudflare, errors = [] }) {
  ...
  // Cloudflare measures the load time of a sample of page loads. At this site's traffic that is a
  // few dozen a week, bunched into two or three days, so the median swings by seconds from one week
  // to the next and a week-on-week change says nothing. Only compare once there are enough of them.
  const ENOUGH_LOAD_TIMES = 100;
  const comparableLoadTime =
    cloudflare?.current.loadTimeSamples >= ENOUGH_LOAD_TIMES &&
    cloudflare?.previous.loadTimeSamples >= ENOUGH_LOAD_TIMES;
  const tiles = [
    {
      label: "Visitors",
    ...
    },
    {
      ...
      change:
        loadTime !== null && comparableLoadTime && change(loadTime, cloudflare.previous.loadTimeMs, { lowerIsBetter: true }),
    },
  ];
  ...
  const lists = vercel
    ? [
        ...
        {
          title: "Top pages",
          rows: vercel.pages.map((r) => ({ name: r.title || r.name, detail: r.title ? r.name : null, value: r.pageviews })),
        },
        {
  ...
  if (vercel && !vercel.trackedWholeWeek) {
    notes.push(
      vercel.days.length
        ? "Vercel Web Analytics started on 15 Sept 2026, so this week is incomplete."
        : "Vercel Web Analytics started on 15 Sept 2026, after this week, so it has no numbers for it."
    );
  }
  if (cloudflare) {
    ...
    notes.push(
      `Cloudflare counted ${number(cf.visits)} visits (${change(cf.visits, cloudflare.previous.visits).text}) and ${number(cf.pageviews)} page views. ` +
        (cf.loadTimeSamples
          ? ` Page load time is Cloudflare's median of ${number(cf.loadTimeSamples)} measured page loads` +
            (comparableLoadTime ? "." : ", too few to compare with the week before.")
          : "") +
  ...
  const text = [
    ...
    ...lists.flatMap((l) => [
      "",
      `${l.title}:`,
      ...l.rows.map((r) => `  ${r.name}${r.detail ? ` (${r.detail})` : ""}: ${number(r.value)}`),
    ]),
    ...(notes.length ? ["", ...notes] : []),
    "",
  ...
  const listHtml = (l) => {
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;table-layout:fixed">${l.rows
      .map(
        (r) => `<tr>
        <td width="50%" style="padding:6px 12px 6px 0;font-size:14px;color:${COLORS.text};line-height:1.35">${r.icon ? `${r.icon}&nbsp; ` : ""}${noAutolink(escapeHtml(r.name))}${
          r.detail
            ? `<div style="font-size:12px;color:${COLORS.muted};word-break:break-all">${noAutolink(escapeHtml(r.detail))}</div>`
            : ""
        }</td>
        <td width="36%" style="padding:6px 0">
